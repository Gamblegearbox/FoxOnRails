...
{	
	...
	private static final float ANIM_SPEED = 50f;
	...
	private static final float[][] muzzleColors = new float[][] { Colors.YELLOW, Colors.WHITE, Colors.PINK };
	...
	public void render(){
		...
		if(isShooting){
			glPointSize(40.0f * (float)Math.random() + 10f);
			glBegin(GL_POINTS);
		...
		glPointSize(22.0f);
		glBegin(GL_POINTS);
			glColor4f(Colors.PINK[0], Colors.PINK[1], Colors.PINK[2], absSinForAnim);
			glVertex3f(0, 0, 1.5f);
		glEnd();
